...
function MessageForm() {
  ...
  return (
      <><div className = "messages-output">{
        ...
        <div className = "alert alert-info" id = "headingchat">{currentRoom} room<
            /div>}
                {user && privateMemberMsg?._id && (
                    <>
       ...
       messages.map(
           ({_id : date, messagesByDate}, idx) => (
               ...
               <p className = "message-sender">{
                   sender._id == user?._id ? "You"
                                           : sender.name}<
                   /p>
                                            </div>
               <img id = "star" src = {staroutline}>
               </img>
                                        </div>
          ...
          /div>
            ...
            <Form onSubmit={handleSubmit}>
                <Row>
                    <Col md={11}>
                        <Form.Group>
                            <Form.Control type="text" placeholder="Your message" disabled={!user} value={message} onChange={(e) => setMessage(e.target.value)}></Form
           .Control></Form.Group>
                    </Col><Col md = {1}>
      <Button variant = "primary" type = "submit" style =
       {
         { width: "100%", backgroundColor: "orange" }
       } disabled = {!user}><i className = "fas fa-paper-plane">
      </i>
                        </Button></Col>
                </Row>
      </Form>
